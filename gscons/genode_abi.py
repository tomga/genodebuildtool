...
class GenodeBaseAbi(genode_target.GenodeTarget):

    def __init__(self, abi_name, env):

        self.abi_name = abi_name
        super().__init__(abi_name, 'abi', 'ABI', env)



class GenodeDisabledAbi(GenodeBaseAbi):

    def __init__(self, abi_name, env, disabled_message):

        super().__init__(abi_name, env)
...
class GenodeAbi(GenodeBaseAbi):

    def __init__(self, abi_name, orig_env, symbols_file_path, symbols_repo, build_env):

        env = orig_env.Clone()
        self.env = env    # avoid cloning environment again in GenodeTarget

        self.symbols_file_path = symbols_file_path
        self.symbols_repo = symbols_repo

        self.build_env = scmkevaluator.ScMkEnv(env,
                                               mk_cache=build_env.mk_cache,
                                               parent_env=build_env)

        self.build_env.var_set('REP_DIR', self.symbols_repo)

        self.build_helper = genode_build_helper.GenodeMkBuildHelper(self.build_env)
        ...
        self.env['ent_current_target_alias'] = self.env['fn_abi_alias_name'](self.abi_name)
    ...
    def norm_tgt_path(self, target):
        if target is not None:
            return '%s/%s/%s' % (self.relative_lib_cache_dir, self.abi_name, target)
        else:
            return '%s/%s' % (self.relative_lib_cache_dir, self.abi_name)
...
##                            (self.lib_name, self.env['fn_localize_path'](self.symbols_file)))
...
##
    ...
    def do_process_target(self):
        ...
        dep_shlib_links = self.env['fn_get_lib_info'](self.abi_name)['common_targets']
...
##        lib_targets.append(lib_tag_tgt)
        ...
        return self.env.Alias(self.env['fn_abi_alias_name'](self.abi_name), abi_targets)
